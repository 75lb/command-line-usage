  ...
  if (definitions.length) {
    if (options.groups) {
      o.each(options.groups, (val, group) => {
        ...
        if (group === '_none') {
          let optionList = getOptionList(definitions.filter(def => !t.isDefined(def.group)))
          output.add(renderSection(null, optionList, true))
        } else {
          let optionList = getOptionList(definitions.filter(def => arrayify(def.group).indexOf(group) > -1))
          output.add(renderSection(null, optionList, true))
        }
      })
    } else {
